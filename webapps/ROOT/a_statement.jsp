...
<%		
	...
	String xmlcnf = "web/students.xml";
	...
	web.setUser(userIP, "students", "studentid", "studentid", userName);
	web.init(request);
	web.setMainPage("a_students.jsp");
	...
	try {
		...
		String dbpath = "jdbc:postgresql://umisdb/ueab";
		Class.forName(driver);
		Connection ldb = DriverManager.getConnection(dbpath, "root", "inventUMIS");
		String mysql = "SELECT accountnumber FROM students WHERE (studentid = '" + request.getRemoteUser() + "')";
		Statement lst = ldb.createStatement(ResultSet.TYPE_SCROLL_SENSITIVE, ResultSet.CONCUR_UPDATABLE);
		ResultSet lrs = lst.executeQuery(mysql);
		if(lrs.next()) accountno = lrs.getString("accountnumber");
		lrs.close();
		lst.close();
		ldb.close();

		driver = "com.microsoft.sqlserver.jdbc.SQLServerDriver";
		dbpath = "jdbc:sqlserver://192.168.3.10;database=SUNPLUSADV;user=online;password=Invent2k;";
		Class.forName(driver);
		...
		mysql = "SELECT SUM(AMOUNT) as ob FROM EDU_A_SALFLDG ";
		...
		// Update the students balance
		mysql = "SELECT upd_sun_balance('" + userName + "', '" + userIP + "', " + ob.toString() + ")";
System.out.println("SUN UPDATE " + mysql);
		web.executeQuery(mysql);
