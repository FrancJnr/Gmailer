...
module.exports = function (sandbox) {
  ...
  bundler.registerHelper('asset_url', function asset_url(name) {
    if (name[0] === '.') {
      name = path.join(N.mainApp.root, name);
    }

    let resolved_path = require.resolve(name);
  ...
  bundler.registerHelper('asset_body', function asset_body(name) {
    if (name[0] === '.') {
      name = path.join(N.mainApp.root, name);
    }
